...
namespace SimpleBT.Editor
{
    ...
    {
        private SBTGraphView _graph; // Required reference in order to add the node to the graph
        private Texture2D _icon; // Icon to add a separation before the entry text.
        private string ParentBehaviorName;
    
        public void Initialize(SBTGraphView graph)
        {
            _graph = graph;
            _icon = new Texture2D(1, 1); // Makes sure it is the minimum available to not separate too much
            _icon.SetPixel(0, 0, Color.clear); // This is currently bugged, Unity Problem.
        }
        
        // This contains all the available nodes in the tool. User data is a string,
        // which gets converted to the proper type upon OnSelectEntry
        ...
        {
            // Filters the status of the root node on the graph view and stops the player if there is no root node
            // before adding a different node or if someone tries to add a second root node after another
            bool rootLocated = false;
            
            if ((string)searchTreeEntry.userData == "RootGraphNode")
            {
                _graph.graphElements.ForEach(element => {
                    if (element is not RootGraphNode) return;
                    rootLocated = true;
                });

                if (rootLocated == true) { EditorUtility.DisplayDialog("Error", "There is already a Root node in the graph!", "OK"); return false; }
            }
            else
            {
                _graph.graphElements.ForEach(element => {
                ...
                if (rootLocated == false) { EditorUtility.DisplayDialog("Error", "There is no Root node! Add it first!", "OK"); return false; }
            }
            
            // Setting up values to generate the appropriate selected option
            ...
            // Determines if the node is or not custom. All custom nods always require user data to contain "Custom_" at the beginning
            // if there is no type then it returns nothing
            ...
            if (type == null)
            {
                EditorUtility.DisplayDialog(
                    "Error", 
                    $"There is no node of type {dataString}," +
                        "if it's a custom node make sure to add Custom_ " +
                        "at the beginning of the user data in your CustomScriptable!", 
                    "OK");
                return true;
            }
            
            // Prepares the node before generation.
            ...
            node.Instantiate(); // Sets a GUID
            node.SetPosition(new Rect(localMousePosition, Vector2.zero));
            node.Set(); // Modifies the node's Title
